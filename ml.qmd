...
#--Map
rf_fin <- rf_test_results %>%
  left_join(asb_test) %>%
  mutate(error = n - `.pred`) %>%
  mutate(pred_popup = paste0("Predicted: ", as.character(round(`.pred`, 1))))

pal <- colorNumeric(palette = c("green", "red"), domain = rf_fin$.pred)
pal_error <- colorNumeric(palette = c("green", "red"), domain = rf_fin$error)

m_rf <- leaflet(rf_fin) |> 
    leaflet::addPolygons(data = bnt_shp, fillOpacity = 0, col = "black") %>%
    leaflet::addTiles() |>
    leaflet::addCircles(
      color = ~pal(rf_fin$.pred),
      popup = ~pred_popup,
      opacity = 0.6) |>
...
m_rf_error <- leaflet(rf_fin) |> 
    leaflet::addPolygons(data = bnt_shp, fillOpacity = 0, col = "black") %>%
    leaflet::addTiles() |>
    leaflet::addCircles(
      color = ~pal_error(rf_fin$error),
      opacity = 0.6) |>
    leaflet::addLegend('bottomright',
            pal =pal_error,
            values = ~error,
...
#saveRDS(m_rf, here("1_data", "m_rf.RDS"))
#saveRDS(m_rf_error, here("1_data", "m_rf_error.RDS"))
